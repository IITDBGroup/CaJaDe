...
def main():
    df=pd.read_csv('scalability_mimic.csv')

    pl.ioff()
    
    ax=df.plot.bar(x='dbscale', y='total', rot=0, color='dodgerblue')
    ...
    ax.set_xlabel('MIMIC DB (#row 1.0=1.1Million)', fontsize=25)
    ...
    pl.show()
    pl.savefig("scalability_mimic.pdf", bbox_inches='tight')
