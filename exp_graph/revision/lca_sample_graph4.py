...
def main():
    ...
    # DFs
    apt1=df.query('APT==4 and is_ref!=1')
    apt1_gt = df.query('APT==4 and is_ref==1')
    ...
    ax1.yaxis.set_ticks(np.arange(0,800, 50))
    ...
    plt.legend(handles, labels, prop={'size': 20}, loc=4, borderpad=0,labelspacing=0,
        handlelength=1,handletextpad=0.2,columnspacing=0.5)

    plt.show()

    plt.savefig("lca_apt4.pdf", bbox_inches='tight')
