...
############################################
############################################
## Plot for CaJaDe_new - Each # stop has six different rates
rate_list = ['10', '09', '08', '07', '06', '05']
stop_list = [1, 3, 5, 7, 9]
marker_shape = ['o', 's', 'v', 'p', '*','D']
marker_color = ['b','g','r','c','m','k']

#sub_plots = plt.subplots(2,3,figsize=(15,15))
#fig = sub_plots[0]
#graph = sub_plots[1]
...
for k in range(0,len(stop_list)): 
  #plt.subplot(2,3,k+1)
  sub_plots = plt.subplots(2,3,figsize=(15,15))
  fig = sub_plots[0]
  graph = sub_plots[1]
  ...
  for i in range(0,len(rate_list)):
    ...
    new_ptt_y = []
    new_jg_total_time = 0
    schema_name = "testing_"+str(rate_list[i])+"_"+str(stop_list[k])
    ...
    if k<3:
      #graph[0][k].figure(figsize=(10,5))
      graph[0][k].plot(new_ptt_x, new_ptt_y, marker_color[i]+marker_shape[i]+'-')
      graph[0][k].set_title('stop #'+str(stop_list[k])+' result')
      graph[0][k].set_xlabel('time(sec)')
      graph[0][k].set_ylabel('# explanation')
    else:
      #graph[1][k].figure(figsize=(10,5))
      graph[1][k-3].plot(new_ptt_x, new_ptt_y, marker_color[i]+marker_shape[i]+'-')
      graph[1][k-3].set_title('stop #'+str(stop_list[k])+' result')
...
#plt.title('Runtime experiment of the CaJaDE system')
  ...
  plt.savefig('stop_'+str(stop_list[k])+'_results.png')
  conn.commit()
