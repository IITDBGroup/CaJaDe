...
for k in range(0,len(rate_list)):
  ...
  for i in range(0,len(stop_list)):
    ...
    if k<3:
      graph[0][k].plot(new_jg_x, new_jg_y, plot_option[i]+'--')
    else:
      graph[1][k-3].plot(new_jg_x, new_jg_y, plot_option[i]+'--')
    ...
    if k<3:
      ...
      graph[0][k].plot(new_ptt_x, new_ptt_y, plot_option[i]+'-')
      graph[0][k].set_title(schema_name+' result')
      graph[0][k].set_xlabel('time(sec)')
      graph[0][k].set_ylabel('# explanation')
    else:
      #graph[1][k].figure(figsize=(10,5))
      graph[1][k-3].plot(new_ptt_x, new_ptt_y, plot_option[i]+'-')
      graph[1][k-3].set_title(schema_name+' result')
