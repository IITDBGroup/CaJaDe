...
creat_lang="CREATE OR REPLACE LANGUAGE plpython2u;"
...
dataCols =  map(lambda x: colLst(table + "_sample",x).rstrip(","), columns)
summCols =  map(lambda x: colLst(table+"_summary",x).rstrip(","), columns)
def matchCl(dcol,scol):
    return " ("+scol+" IS NULL OR "+scol+" = "+dcol+") and"
where = reduce(add,map(matchCl,dataCols,summCols)).rstrip("and")
...
estCols =  map(lambda x: colLst(table+"_sample"+"_estimate",x).rstrip(","), columns)
...
print "TOTAL TIME: ", time.time()-totaltimer
